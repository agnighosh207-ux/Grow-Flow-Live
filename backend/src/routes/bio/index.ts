  ...
  try {
    ...
    if (!filterResult.allowed) {
      res.status(400).json({ 
        ...
        suggestion: filterResult.suggestion || "Please provide a specific main topic idea"
      });
      return;
    }

    const sanitizedMainTopic = filterResult.cleanedInput || String(mainTopic);
