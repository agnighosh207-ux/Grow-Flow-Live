...
const formats = [
 ...
 { id: "newsletter", label: "Newsletter", icon: <Mail className="h-6 w-6" />, color: "from-indigo-600 to-[#0A0A0F] shadow-[rgba(94,106,210,0.2)]" },
];
 ...
 return (
  <PageWrapper maxWidth="xl" className="space-y-12 pb-24 md:pb-8">
   <FeatureGuideBanner 
    ...
    icon={<Share2 className="w-5 h-5 text-[#8B91E3]" />}
   ...
   <div className="flex flex-col gap-8">
    <PageHeader 
     ...
     action={
      <div className="hidden md:flex items-center gap-2 bg-white/5 p-1.5 rounded-2xl border border-white/10 backdrop-blur-3xl shadow-2xl">
        {[1, 2, 3].map(i => (
         <div key={i} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all duration-500 ${step === i ? 'bg-[#5E6AD2] text-white font-black shadow-lg scale-105' : 'text-white/20'}`}>
          <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black ${step === i ? 'bg-white text-[#5E6AD2]' : 'bg-white/5 text-white/20'}`}>{i}</div>
    ...
    <div className="flex md:hidden items-center justify-between gap-2 bg-white/5 p-1.5 rounded-2xl border border-white/10 overflow-x-auto no-scrollbar">
      {[1, 2, 3].map(i => (
       <div key={i} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all duration-500 shrink-0 ${step === i ? 'bg-[#5E6AD2] text-white font-black' : 'text-white/20'}`}>
        <div className={`w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-black ${step === i ? 'bg-white text-[#5E6AD2]' : 'bg-white/5 text-white/20'}`}>{i}</div>
    ...
    {step === 2 && (
     ...
     >
       ...
       <div className="flex gap-6 max-w-3xl mx-auto pt-10">
        ...
        <Button 
         className="flex-1 h-20 bg-gradient-to-r from-indigo-600 to-indigo-900 hover:from-indigo-500 hover:to-indigo-900 text-white font-black text-2xl rounded-3xl shadow-2xl shadow-indigo-600/30 group transition-all"
    ...
    {step === 3 && result && (
     ...
     >
             ...
             <CardContent className="p-16 space-y-12">
               ...
               <div className="pt-12 border-t border-white/5 flex flex-col xl:flex-row justify-between items-center gap-10">
                <div className="flex items-start gap-5 max-w-2xl">
                  <div className="p-3 rounded-2xl bg-white/5 " style={{ color: "var(--text-disabled)" }}><Layers className="h-6 w-6" /></div>
                ...
                <div className="flex items-center gap-6 shrink-0">
                  <div className="text-right hidden sm:block">
                   <p className="text-xs font-black uppercase tracking-widest" style={{ color: "var(--text-disabled)" }}>Payload Size</p>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto pt-10">
        ...
        <Button 
         className="h-20 bg-gradient-to-r from-[#5E6AD2] to-indigo-500 text-white font-black text-xl rounded-3xl shadow-2xl shadow-[rgba(94,106,210,0.30)] hover:scale-[1.02] transition-all"
