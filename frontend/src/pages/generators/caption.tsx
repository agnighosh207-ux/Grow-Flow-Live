    ...
    <div className="mt-3 pt-3 border-t border-white/6 space-y-1">
     {changes.map((c, i) => (
      <div key={i} className="flex items-start gap-1.5 text-[11px] text-white/40">
       <ChevronRight className="w-3 h-3 mt-0.5 shrink-0 text-[rgba(139,145,227,0.60)]" />
       {c}
      </div>
     ))}
    </div>
 ...
 return (
  <PageWrapper maxWidth="sm" className="pb-24 md:pb-8 space-y-8">
    <FeatureGuideBanner 
     ...
     icon={<Wand2 className="w-5 h-5 text-[#8B91E3]" />}
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
      ...
      <div className="flex justify-between mt-1">
       <div className="flex items-center gap-2">
        ...
        {platform === "Twitter" && caption.match(/https?:\/\/[^\s]+/) && (
         <span className="text-[9px] text-[rgba(139,145,227,0.50)] font-medium tracking-tight">
      ...
      <div>
       ...
       <div className="relative">
        <select
         ...
         className="w-full px-3 py-2.5 rounded-xl text-white text-sm focus:outline-none focus:border-[rgba(94,106,210,0.50)] appearance-none cursor-pointer" style={{ background: "var(--surface-2)", border: "1px solid var(--border)", color: "#ffffff" }}
        >
       ...
       {IMPROVEMENT_FOCUS_OPTIONS.map((opt) => (
        <button
         ...
         className={`text-left px-3 py-2 rounded-xl border text-xs transition-all leading-snug ${
          focus === opt.value
           ? "bg-[rgba(94,106,210,0.15)] border-[rgba(94,106,210,0.40)] text-[#8B91E3]"
     ...
     <button
      ...
      className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl bg-gradient-to-r from-[#5E6AD2] to-indigo-600 hover:from-[#5E6AD2] hover:to-indigo-500 text-white font-semibold text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
     >
      {loading ? (
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
       ...
       {result.hookScore && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
         className="rounded-2xl bg-white/[0.03] border border-white/8 p-4">
         <div className="flex items-center gap-2 mb-3">
          <TrendingUp className="w-4 h-4 text-[#8B91E3]" />
       ...
       {activeTab === "full" && result.fullRewrite && (
        <CaptionCard
         ...
         accent="bg-[rgba(94,106,210,0.12)] text-[#8B91E3] border-[rgba(94,106,210,0.20)]"
         delay={0.2}
        />
       )}
