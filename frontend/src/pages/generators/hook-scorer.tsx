   ...
   <div className="max-w-2xl mx-auto space-y-4">
    ...
    <div className="rounded-2xl border border-white/8 bg-white/[0.02] p-4 space-y-3">
     <textarea
      ...
      className="w-full rounded-xl p-3 text-white text-sm placeholder:text-white/25 focus:outline-none focus:border-[rgba(94,106,210,0.40)] resize-none min-h-[80px]" style={{ background: "var(--surface-2)", border: "1px solid var(--border)" }}
      maxLength={300}
     />
     <div className="flex items-center gap-3">
      <select
       ...
       className="flex-1 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-[rgba(94,106,210,0.40)]" style={{ background: "var(--surface-2)", border: "1px solid var(--border)" }}
      >
      ...
      <button
       ...
       className="bg-[#5E6AD2] hover:bg-[#5E6AD2] disabled:opacity-40 text-white font-bold px-6 py-2 rounded-xl text-sm transition-all flex items-center gap-2 shadow-lg shadow-[rgba(94,106,210,0.40)]"
      >
    ...
    {result && (
     <div className="space-y-3">
      ...
      {result.improvedVersion && (
       <div className="rounded-2xl border border-[rgba(94,106,210,0.20)] bg-[rgba(94,106,210,0.5)] p-4">
        <div className="flex items-center justify-between mb-2">
         <h3 className="text-xs font-black text-[#8B91E3] uppercase tracking-widest">AI-Improved Version</h3>
         <button onClick={() => { navigator.clipboard.writeText(result.improvedVersion); toast({ title: "Copied!" }); }}
          className="text-xs text-white/30 hover:text-[#8B91E3] flex items-center gap-1">
