      ...
      days.push(
        ...
        >
          <div className="flex items-center justify-between">
            ...
            {itemsForDay.length > 0 && <span className="w-1.5 h-1.5 rounded-full bg-cyan-500 shadow-[0_0_8px_rgba(139,92,246,0.8)]" />}
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pointer-events-auto">
            {itemsForDay.map((item) => (
              <div 
                key={item.id} 
                className="text-[10px] leading-tight flex flex-col rounded-md p-1.5 border border-white/5 bg-black/20 hover:bg-black/40 hover:border-cyan-500/30 transition-all cursor-pointer group"
              >
                <div className="flex justify-between items-center mb-1">
                  <span className="font-semibold text-cyan-300 drop-shadow flex items-center gap-1">
            ...
            <>
              <div className="grid grid-cols-7 gap-4 mb-4">
                {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
                  <div key={day} className="text-center text-xs font-bold uppercase tracking-widest text-cyan-300/70">
                    {day}
                  </div>
                ))}
              </div>
