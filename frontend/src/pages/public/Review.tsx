...
export default function ReviewPage() {
  ...
  return (
    <div className="min-h-screen bg-[#050110] selection:bg-[rgba(94,106,210,0.30)] font-sans">
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        <div className="mb-12">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[rgba(94,106,210,0.10)] border border-[rgba(94,106,210,0.20)] mb-4">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-[#8B91E3]">Review Request</span>
          </div>
              ...
              generatedData[p] && (
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-xs font-bold transition-all whitespace-nowrap
                    ${activeTab === p 
                      ? "bg-[rgba(94,106,210,0.10)] text-[#8B91E3] border border-[rgba(94,106,210,0.20)] shadow-[0_0_15px_rgba(94,106,210,0.05)]" 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {generatedData.instagram.visualBriefs.map((brief: string, idx: number) => (
                            <div key={idx} className="bg-white/5 border border-white/10 rounded-xl p-4 text-[11px] text-white/60">
                              <span className="text-[#8B91E3] font-bold mr-2">Slide {idx + 1}:</span> {brief}
                            </div>
                          ))}
                        </div>
                      </div>
        ...
        {!submitted ? (
          <motion.div
            ...
            className="bg-[#0A061E] border border-white/10 rounded-[32px] p-8 md:p-10 shadow-2xl shadow-[rgba(94,106,210,0.20)]"
          >
            <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
              <MessageSquare className="w-5 h-5 text-[#8B91E3]" />
              Share your feedback
            </h3>
            ...
            <div className="space-y-4">
              <textarea
                ...
                className="w-full bg-black/40 border border-white/10 rounded-2xl p-6 text-sm text-white placeholder-white/20 focus:outline-none focus:border-[rgba(94,106,210,0.50)] transition-all min-h-[120px] resize-none font-medium"
              />
              <Button
                onClick={handleSubmitFeedback}
                disabled={!feedbackStatus || isSubmitting}
                className="w-full h-14 bg-[#5E6AD2] hover:bg-[#5E6AD2] text-white font-bold rounded-2xl shadow-xl shadow-[rgba(94,106,210,0.40)] disabled:opacity-50 disabled:cursor-not-allowed group"
              >
        ...
        <div className="mt-20 pt-12 border-t border-white/[0.06] text-center">
          ...
          <div className="inline-flex flex-col md:flex-row items-center gap-4 bg-white/[0.03] border border-white/10 p-2 rounded-3xl">
            <div className="flex -space-x-3 px-4">
              ...
              <div className="w-10 h-10 rounded-full border-2 border-[#050110] bg-[#5E6AD2] flex items-center justify-center text-[10px] font-black text-black">
                +420
              </div>
            </div>
            ...
            <Button
              onClick={() => window.location.href = "https://growflowai.space"}
              className="bg-[#5E6AD2] hover:bg-[#5E6AD2] text-white font-black text-xs px-6 py-4 rounded-2xl flex items-center gap-2 group shadow-xl shadow-[rgba(94,106,210,0.3)]-950"
            >
